...
// New Critical Features Models
const AuditChecklist = require('./AuditChecklist');
const QualityAudit = require('./QualityAudit');
const PolicyUpdate = require('./PolicyUpdate');
const SelfAuditReport = require('./SelfAuditReport');
const PeerComparison = require('./PeerComparison');
const ScenarioSimulation = require('./ScenarioSimulation');
const GovernmentPortalIntegration = require('./GovernmentPortalIntegration');
const NotificationEscalation = require('./NotificationEscalation');
...
const defineAssociations = () => {
  ...
  // New Critical Features Associations
  
  // AuditChecklist Associations
  Institution.hasMany(AuditChecklist, { foreignKey: 'institutionId', as: 'auditChecklists' });
  AuditChecklist.belongsTo(Institution, { foreignKey: 'institutionId', as: 'institution' });

  User.hasMany(AuditChecklist, { foreignKey: 'createdBy', as: 'createdChecklists' });
  AuditChecklist.belongsTo(User, { foreignKey: 'createdBy', as: 'creator' });

  User.hasMany(AuditChecklist, { foreignKey: 'reviewedBy', as: 'reviewedChecklists' });
  AuditChecklist.belongsTo(User, { foreignKey: 'reviewedBy', as: 'reviewer' });

  // QualityAudit Associations
  Institution.hasMany(QualityAudit, { foreignKey: 'institutionId', as: 'qualityAudits' });
  QualityAudit.belongsTo(Institution, { foreignKey: 'institutionId', as: 'institution' });

  User.hasMany(QualityAudit, { foreignKey: 'userId', as: 'qualityAudits' });
  QualityAudit.belongsTo(User, { foreignKey: 'userId', as: 'user' });

  // PolicyUpdate Associations
  Institution.hasMany(PolicyUpdate, { foreignKey: 'institutionId', as: 'policyUpdates' });
  PolicyUpdate.belongsTo(Institution, { foreignKey: 'institutionId', as: 'institution' });

  User.hasMany(PolicyUpdate, { foreignKey: 'userId', as: 'policyUpdates' });
  PolicyUpdate.belongsTo(User, { foreignKey: 'userId', as: 'user' });

  User.hasMany(PolicyUpdate, { foreignKey: 'reviewedBy', as: 'reviewedPolicies' });
  PolicyUpdate.belongsTo(User, { foreignKey: 'reviewedBy', as: 'reviewer' });

  // SelfAuditReport Associations
  Institution.hasMany(SelfAuditReport, { foreignKey: 'institutionId', as: 'selfAuditReports' });
  SelfAuditReport.belongsTo(Institution, { foreignKey: 'institutionId', as: 'institution' });

  User.hasMany(SelfAuditReport, { foreignKey: 'userId', as: 'selfAuditReports' });
  SelfAuditReport.belongsTo(User, { foreignKey: 'userId', as: 'user' });

  User.hasMany(SelfAuditReport, { foreignKey: 'reviewerId', as: 'reviewedReports' });
  SelfAuditReport.belongsTo(User, { foreignKey: 'reviewerId', as: 'reviewer' });

  // PeerComparison Associations
  Institution.hasMany(PeerComparison, { foreignKey: 'institutionId', as: 'peerComparisons' });
  PeerComparison.belongsTo(Institution, { foreignKey: 'institutionId', as: 'institution' });

  User.hasMany(PeerComparison, { foreignKey: 'userId', as: 'peerComparisons' });
  PeerComparison.belongsTo(User, { foreignKey: 'userId', as: 'user' });

  // Self-referencing for target and peer organizations
  Institution.hasMany(PeerComparison, { foreignKey: 'targetOrganizationId', as: 'targetComparisons' });
  PeerComparison.belongsTo(Institution, { foreignKey: 'targetOrganizationId', as: 'targetOrganization' });

  // ScenarioSimulation Associations
  Institution.hasMany(ScenarioSimulation, { foreignKey: 'institutionId', as: 'scenarioSimulations' });
  ScenarioSimulation.belongsTo(Institution, { foreignKey: 'institutionId', as: 'institution' });

  User.hasMany(ScenarioSimulation, { foreignKey: 'userId', as: 'scenarioSimulations' });
  ScenarioSimulation.belongsTo(User, { foreignKey: 'userId', as: 'user' });

  // GovernmentPortalIntegration Associations
  Institution.hasMany(GovernmentPortalIntegration, { foreignKey: 'institutionId', as: 'portalIntegrations' });
  GovernmentPortalIntegration.belongsTo(Institution, { foreignKey: 'institutionId', as: 'institution' });

  User.hasMany(GovernmentPortalIntegration, { foreignKey: 'userId', as: 'portalIntegrations' });
  GovernmentPortalIntegration.belongsTo(User, { foreignKey: 'userId', as: 'user' });

  // NotificationEscalation Associations
  Institution.hasMany(NotificationEscalation, { foreignKey: 'institutionId', as: 'notificationEscalations' });
  NotificationEscalation.belongsTo(Institution, { foreignKey: 'institutionId', as: 'institution' });

  User.hasMany(NotificationEscalation, { foreignKey: 'userId', as: 'notificationEscalations' });
  NotificationEscalation.belongsTo(User, { foreignKey: 'userId', as: 'user' });

  // Cross-feature integrations
  // AuditChecklist can be linked to QualityAudits
  AuditChecklist.hasMany(QualityAudit, { foreignKey: 'checklistId', as: 'qualityAudits' });
  QualityAudit.belongsTo(AuditChecklist, { foreignKey: 'checklistId', as: 'checklist' });

  // SelfAuditReports can reference PolicyUpdates
  PolicyUpdate.hasMany(SelfAuditReport, { foreignKey: 'policyReferenceId', as: 'referencedReports' });
  SelfAuditReport.belongsTo(PolicyUpdate, { foreignKey: 'policyReferenceId', as: 'referencedPolicy' });

  // ScenarioSimulations can be triggered by NotificationEscalations
  NotificationEscalation.hasMany(ScenarioSimulation, { foreignKey: 'triggeredByEscalationId', as: 'triggeredSimulations' });
  ScenarioSimulation.belongsTo(NotificationEscalation, { foreignKey: 'triggeredByEscalationId', as: 'triggeringEscalation' });

  // PeerComparisons can reference ScenarioSimulations
  ScenarioSimulation.hasMany(PeerComparison, { foreignKey: 'simulationId', as: 'referencedComparisons' });
  PeerComparison.belongsTo(ScenarioSimulation, { foreignKey: 'simulationId', as: 'scenarioSimulation' });

  // GovernmentPortalIntegration can be referenced by QualityAudits
  GovernmentPortalIntegration.hasMany(QualityAudit, { foreignKey: 'portalIntegrationId', as: 'qualityAudits' });
  QualityAudit.belongsTo(GovernmentPortalIntegration, { foreignKey: 'portalIntegrationId', as: 'portalIntegration' });
};
